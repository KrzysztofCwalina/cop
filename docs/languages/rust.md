...
> "Add a cop check that flags `panic!`, `unwrap()`, and `expect()` in library code — propagate errors with `?`"

> "Add a cop check that bans `println!` and `dbg!` — use the `log` or `tracing` crate"
